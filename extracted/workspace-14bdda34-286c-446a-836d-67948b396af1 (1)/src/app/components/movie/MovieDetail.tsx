...
export default function MovieDetail({ movie, isAdmin }: MovieDetailProps) {
  ...
  // Combine all download links (movie + all episodes) on mount
  useEffect(() => {
    const allLinks: DownloadLink[] = [...(movie.downloadLinks || [])];
    
    // Add episode download links for series
  ...
  // Create stored movie object for bookmark/recent
  ...
  // Filter out Season 0 (Specials) - only show real seasons
  const validSeasons = useMemo(() => 
    movie.seasons.filter(s => s.seasonNumber > 0).sort((a, b) => a.seasonNumber - b.seasonNumber),
    [movie.seasons]
  );

  // Auto-select first valid season for series
  useEffect(() => {
  ...
  // Mark as viewed when component mounts (for Recent)
  useEffect(() => {
    if (!viewed) {
  ...
  // Fetch related movies for Explore tab
  useEffect(() => {
                    ...
                    <Link key={relatedMovie.id} href={relatedMovie.type === 'series' ? `/series/${relatedMovie.id}` : `/movie/${relatedMovie.id}`} className="group">
                      <div className="aspect-[2/3] relative rounded overflow-hidden bg-[#1E1E1E]">
                        {relatedMovie.posterPath ? (
                          <Image src={`https://image.tmdb.org/t/p/w300${relatedMovie.posterPath}`} alt={relatedMovie.title || 'Movie'} fill className="object-cover group-hover:scale-105 transition-transform" unoptimized />
                        ) : (
